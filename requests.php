...
function updateGuideIdInGroupMembers($conn, $guide_id, $student_id, $group_id) {
    // Start a transaction
    mysqli_begin_transaction($conn);

    // Fetch all student_ids in the same group
    $fetchGroupMembersQuery = "SELECT student_id FROM group_members WHERE group_id = ?";
    $stmtFetchGroupMembers = mysqli_prepare($conn, $fetchGroupMembersQuery);
    mysqli_stmt_bind_param($stmtFetchGroupMembers, 'i', $group_id);
    mysqli_stmt_execute($stmtFetchGroupMembers);
    mysqli_stmt_bind_result($stmtFetchGroupMembers, $student_id_in_group);

    $studentsToUpdate = [];
    while (mysqli_stmt_fetch($stmtFetchGroupMembers)) {
        $studentsToUpdate[] = $student_id_in_group;
    }
    mysqli_stmt_close($stmtFetchGroupMembers);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'], $_POST['request_id'])) {
    ...
    if (mysqli_stmt_fetch($stmtGetRequestData)) {
        ...
        if ($existing_guide_id === null || $existing_group_id === null) {
            ...
            if (updateGuideIdInGroupMembers($conn, $guide_id, $student_id, $group_id)) {
...
$guide_id = $_SESSION['user_id'];
$requestsQuery = "SELECT r.id AS request_id, r.student_id, r.group_id, u.name AS student_name, r.created_at AS request_date
                  FROM requests r
...
if ($stmt) {
    ...
    mysqli_stmt_bind_result($stmt, $request_id, $student_id, $group_id, $student_name, $request_date);
