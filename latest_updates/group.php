...
<body>
            ...
            <tbody id="studentList">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        // Check if student is already grouped
                        $grouped_sql = "SELECT student_id FROM student_groups WHERE student_id = '" . $row["registerNo"] . "'";
                        $grouped_result = $conn->query($grouped_sql);
                        if ($grouped_result->num_rows > 0) {
                            echo "<td><input type='checkbox' name='selectedStudents[]' value='" . $row["registerNo"] . "' disabled></td>";
                        } else {
    ...
    <script>
        ...
        function enableDisableFormGroupButton() {
            ...
            formGroupBtn.disabled = !checkIfAnyCheckboxChecked();
        }
                    ...
                    if (this.status == 200) {
                        try {
                            ...
                            if (response.success) {
                                ...
                                // Disable Form Group button
                                document.getElementById("formGroupBtn").disabled = true;
                            } else {
                                // Error message
