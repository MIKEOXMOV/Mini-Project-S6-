...
$student_query = "SELECT u.id, u.name, u.email, u.register_or_faculty_id, sg.group_id
                  FROM users u
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert or update student marks into student_marks table
    $sql = "INSERT INTO student_marks (student_id, project_id, attendance, project_report, review1_total_marks, review2_total_marks, average_review_marks, final_cie_mark)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
            attendance = VALUES(attendance),
            project_report = VALUES(project_report),
            review1_total_marks = VALUES(review1_total_marks),
            review2_total_marks = VALUES(review2_total_marks),
            average_review_marks = VALUES(average_review_marks),
            final_cie_mark = VALUES(final_cie_mark)";
    ...
    $guide_marks_query = "SELECT guide_marks FROM guide_marks WHERE student_id = ? AND project_id = ?";
    ...
    if ($guide_marks_result->num_rows > 0) {
        ...
        $guide_marks = $guide_marks_row['guide_marks'];
    }
...
<body>
    <nav class="navbar navbar-light bg-light">
        ...
        <a href="display_project.php" class="btn btn-outline-primary back-button">Back</a>
    </nav>

    <div class="container mt-5">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title">Evaluate Student</h5>
                <h6>Project: <?php echo htmlspecialchars($project['name']); ?></h6>
                <h6>Student: <?php echo htmlspecialchars($student['name']); ?></h6>
                <h6>Email: <?php echo htmlspecialchars($student['email']); ?></h6>
                <h6>Register ID: <?php echo htmlspecialchars($student['register_or_faculty_id']); ?></h6>
                ...
                <form action="" method="POST" class="mt-4" oninput="calculateTotalCieMarks()">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="number" class="form-control" id="guide_marks" name="guide_marks" max="15" value="<?php echo $guide_marks; ?>" disabled>
                        </div>
                    </div>
                    <div class="form-group row">
